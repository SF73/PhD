# -*- coding: utf-8 -*-
"""
Created on Mon Nov 25 13:01:08 2019
...
cs_srgb = ColourSystem(red=xyz_from_xy(0.64, 0.33),
                       ...
                       blue=xyz_from_xy(0.15, 0.06),
                       white=illuminant_D65)
def spec_interpolation(wavelenght,spec,filter=False):
    if filter:
        from scipy.signal import savgol_filter
        spec = savgol_filter(spec,101,2)
    f = interpolate.interp1d(wavelenght,spec)
    return f(np.linspace(380,780,81))

def to_rgb(wavelenght, hyp):
    #CLdata[:,n] : n numero du spectre
    #self.hypSpectrum = np.transpose(np.reshape(np.transpose(CLdata),(ylen,xlen ,len(self.wavelenght))), (0, 1, 2))
    cs = cs_smpte
    temp = hyp.reshape(-1, hyp.shape[-1])
    rgbimage = np.zeros((temp.shape[0],3))
    for i in range(temp.shape[0]):
        spec = spec_interpolation(temp[i])
        rgbimage[i]=cs.spec_to_rgb(spec)
    return np.reshape(rgbimage,(hyp.shape[0],hyp.shape[1],3))
